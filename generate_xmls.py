...
class ElasticSearchClient:
    ...
    def extract_all_data_from_es(self, es_index, url):
        output_list = []
        start_time = time.time()
        ...
        if len(dict_data) > 0:
            ...
            if len(emails_df) > 0:
                ...
                for title_idx, title in tqdm(enumerate(titles)):
                    ...
                    # title_df.to_csv(f"title_df_{title_idx}.csv", index=False)
                    if len(title_df) < 1:
                        continue
                    if len(title_df) == 1:
...
if __name__ == "__main__":
    ...
    for dev_url in dev_urls:
        data_list = elastic_search.extract_all_data_from_es(ES_INDEX, dev_url)
