...
class XMLThreadingUpdater:
    ...
    def get_threading_data_for_title_with_retry(self, title, dev_url, max_retries=3):
    ...
    def update_all_threading(self, start_year=None, start_month=None, months_limit=None):
        """Update all combined XML files with threading structure - FIXED VERSION"""
        ...
        for i, xml_file_path in enumerate(files_to_process):
            try:
                logger.info(f"📁 PROGRESS: Processing file {i+1}/{len(files_to_process)}: {xml_file_path}")
                ...
                thread_data = self.get_threading_data_for_title_with_retry(title, dev_url)
                ...
                if i < len(files_to_process) - 1:  # Don't sleep after last file
                    logger.info("⏸️ Waiting 2 seconds to avoid ES overload...")
                    time.sleep(2)
                
                # Progress report every 10 files
                if (i + 1) % 10 == 0:
                    logger.info(f"📊 PROGRESS: {i+1}/{len(files_to_process)} files processed. Updated: {updated_count}, Skipped: {skipped_count}, Errors: {error_count}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        start_month = sys.argv[2] if sys.argv[2] else None
    if len(sys.argv) > 3:
        months_limit = sys.argv[3] if sys.argv[3] else None
